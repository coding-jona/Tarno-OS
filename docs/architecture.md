...
### Tarno AI (Phase 1: heuristisch + Phase 2: Mistral-API, erster Cut)
- Modul `tarnod/tarnod/src/ai/` (Geschwister von `gaming.rs`, `security/`, `vault.rs`), in `AppState` als `ai: ai::AiState` eingebunden.
- `ai/backend.rs`: `#[async_trait] trait AiBackend: Send + Sync { async fn answer(&self, question: &str, ctx: &SystemContext) -> String; }` (seit Phase 2 `async`, da ein Netzwerk-Request nicht blockierend im IPC-Handler laufen darf) + `SystemContext` (Gaming-Mode-Status/Governor, isolierte CPUs, `ebpf`-Feature-Status, RAM aus `/proc/meminfo`).
...
- `ai/mistral.rs`: Phase-2-Backend `MistralBackend` — direkter `reqwest`-Aufruf (rustls, kein System-OpenSSL) gegen `https://api.mistral.ai/v1/chat/completions`, Key aus der `Vault` (`MISTRAL_API_KEY`), Retry/Backoff bei 429/5xx, fester Rate-Limiter (0.8 req/s).
...
- IPC: `Request::AiQuery{text}`, `Request::AiStatus` (inkl. `mistral_configured`/`mode_note`), `Request::AiSuggestions` in `tarnod-protocol`, per `tarnoctl ai <status|suggestions|<frage...>>` erreichbar.
- Details, Testabdeckung, Setup-Anleitung und der ehrliche Phase-2/3-Stand (Vereinfachungen ggü. der Python-Referenz, Phase-3-Ausblick Security-Event-Integration): [`month3-tarno-layer.md`](month3-tarno-layer.md#tarno-ai).
